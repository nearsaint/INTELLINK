  ...
  <div v-show="model.show" class="model">
    ...
    <div class="modelBox  animated fadeInDown">
      ...
      <div class="footer ">
        ...
        <el-button type="primary " v-show="model.step==0" size="medium " :loading="this.model.btnLoading_writeCard" @click="writeCard()">下一步</el-button>
...
export default {
  ...
  methods: {
    ...
    getTree: function(id) {
      let that = this;
      that.axios.get('/api/card/GetPermission?Id=' + id)
        .then(function(rel) {
          if (rel.status == 200) {
            console.log('获取权限树');
            console.log(rel);
            that.ceartTree(rel.data.Data);
          }
        }).catch(rel => {
          console.log(rel);
          that.$message({
    ...
    //创建树
    ceartTree: function(dataOne) {
      console.log(dataOne);
      let treeOne = $("#treeOne");
      treeOne = $.fn.zTree.init(treeOne, this.model2.setting, dataOne);
    },
    getChecked: function() {
      let that = this;
      ...
      for (let ione = 0; ione < treeOneChecked.length; ione++) {
        if (!treeOneChecked[ione].hasOwnProperty('isParent')) {
          that.model2.checked.push(treeOneChecked[ione].id);
        }
      }
      console.log(that.model2.checked);
    },
    submit_q: function() {
      this.getChecked();
      let that = this;
      ...
      that.axios.post('/api/card/AuthorizePermission', {
        ...
        "DoorPermission": that.model2.checked
      }).then(function(rel) {
        console.log(rel);
        that.$message({
          message: "授权成功",
    ...
    writeCard: function() {
      let that = this;
      if (that.model.input.code1 == '' || that.model.input.code2 == '') {
        that.$message({
          type: 'info',
          message: '请正确填写表单!'
        });
